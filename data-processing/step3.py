...
#trich xuat nodes va egdes

joern_output_path = 'output/joern-output/non-vulnerables'
subdirectories = [f for f in os.listdir(
    joern_output_path) if os.path.isdir(os.path.join(joern_output_path, f))]
output_path = 'output/cpg/non-vulnerables'
...
for subdirectory in subdirectories:
    ...
    list_dot_file_path = joern_output_path + f'/{subdirectory}'
    csv_path = os.path.join(output_path, subdirectory)
    if not os.path.exists(csv_path):
        os.mkdir(csv_path)
    else:
        count+=1
        continue
    for dot_file_path in os.listdir(list_dot_file_path):
        nodes, edges = read_dot_file(
            os.path.join(list_dot_file_path, dot_file_path))
        write_to_csv(nodes, edges, csv_path)
    count+=1
